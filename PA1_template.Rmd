...
medday<-median(daytotal)
meanday<-mean(daytotal)

```
The median steps per day is `r medday`.
The mean steps per day is is `r meanday`.
...
# write the interval time of day with maximum average steps
timeofday<-interval[which(intervalavg==max(intervalavg,na.rm=TRUE))]
```
The interval with the maximum average steps taken is `r timeofday`.

### Imputing missing values
#### Given the average steps taken for each interval across all days, fill in the missing step values with the appropriate average value
```{r, echo=TRUE}
...
# calculate mean and median values of this array
medday<-median(daytotalfill)
meanday<-mean(daytotalfill)
```
The median steps per day is `r medday`.
The mean steps per day is `r meanday`.
